...
import 'package:cms_app_v2/Views/profession_details.dart';
  ...
  Future<List<String>> fetchUniqueCompanies() async {
    try {
      ...
      List<String> companies = querySnapshot.docs
          .map((doc) => doc.data()['position'] as String)
          .toList();
                            ...
                            InkWell(
                              onTap: () {
                                Navigator.of(context).push(
                                    MaterialPageRoute(
                                        builder: (context) => ProfessionDetails(profession: companies[index],)));
                              },
                              child: ListTile(
                                leading: Container(
                                  height: height / 12.56,
                                ...
                                subtitle: Row(
                                  children: [
                                    ...
                                    FutureBuilder(
                                        future: FirebaseFirestore.instance.collection('Members').where("position",isEqualTo: companies[index]).get(),
                                        builder: (context, snapshot) {
                                          if (snapshot.hasData) {
                                            return Text(
